...
class GNNLayer(nn.Module):
    ...
    def forward(self, h, e, graph, mode="residual", edge_index=None, sparse=False):
        ...
        h = Uh + self.aggregate(
            Vh, graph, gates, edge_index=edge_index, sparse=sparse
        )  # B x V x H
        ...
        if not sparse:
            if (mode or self.aggregation) == "mean":
                return torch.sum(Vh, dim=2) / (
                    torch.sum(graph, dim=2).unsqueeze(-1).type_as(Vh)
                )
...
class ScalarEmbeddingSine1D(nn.Module):
    ...
    def forward(self, x):
        ...
        pos_x = x_embed[:, :, None] / dim_t  # Shape: (32, 20, 256)
        pos_x = torch.stack(
            (pos_x[:, :, 0::2].sin(), pos_x[:, :, 1::2].cos()), dim=3
        )  # Shape: (32, 20, 128, 2)
        pos_x = pos_x.flatten(2)  # Shape: (32, 20, 256)

        return pos_x


def run_sparse_layer(
    layer, time_layer, out_layer, adj_matrix, edge_index, add_time_on_edge=True
):
    def custom_forward(*inputs):
        ...
        time_emb = inputs[2]
        x, e = layer(
            x_in, e_in, adj_matrix, mode="direct", edge_index=edge_index, sparse=True
        )
        if add_time_on_edge:
...
class GNNEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # n_out_features: number of output classes to predict.
        # Defaults to n_nodes_features when not set (backward-compatible).
        if n_out_features is None:
            n_out_features = n_nodes_features
        self.sparse = sparse
        ...
        self.features_embed = nn.Linear(n_nodes_features, hidden_dim)
        ...
        if not node_feature_only:
            ...
            # Embedding table: input vocab size = n_nodes_features (e.g. 3 for 0/1/mask)
            self.pos_embed = nn.Embedding(n_nodes_features, hidden_dim)
        ...
        # Output head: Linear → ReLU → Linear (no LayerNorm)
        self.out = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, n_out_features)
        )
    ...
    def dense_forward_node_feature_only(self, x, graph, timesteps):
        """
        Args:
            x: Input node coordinates (B x V x 2)
            graph: Graph adjacency matrices (B x V x V)
            ...
            edge_index: Edge indices (2 x E)
        Returns:
            Updated nodes features (B x V x 2)
        """

        x = self.node_embed(self.pos_embed(x.squeeze(-1)))
        x_shape = x.shape
        e = self.edge_embed(graph.float())
        time_emb = self.time_embed(timestep_embedding(timesteps, self.hidden_dim))
        ...
        x = self.out(x)  # .squeeze(0).permute(1,2,0)
    ...
    def sparse_forward_node_feature_only(self, x, timesteps, edge_index, edge_attr):
        """_summary_
        ...
        time_emb = self.time_embed(timestep_embedding(timesteps, self.hidden_dim))
        edge_index = edge_index.long()
        x, e = self.sparse_encoding(x, e, edge_index, time_emb, edge_attr)
        x = self.out(x)
        return x

    def sparse_encoding(self, x, e, edge_index, time_emb, edge_attr):
        adj_matrix = SparseTensor(
            row=edge_index[0],
            col=edge_index[1],
            value=torch.ones_like(edge_attr.float()),
            sparse_sizes=(x.shape[0], x.shape[0]),  # , 4)
        )
        ...
        for layer, time_layer, out_layer in zip(
            ...
            if self.use_activation_checkpoint:
                ...
                run_sparse_layer_fn = functools.partial(
                    run_sparse_layer, add_time_on_edge=not self.node_feature_only
                )

                out = activation_checkpoint.checkpoint(
                    run_sparse_layer_fn(
                        layer, time_layer, out_layer, adj_matrix, edge_index
                    ),
                    x_in,
                    e_in,
                    single_time_emb,
    ...
    def forward(self, x, timesteps, graph=None, edge_index=None, edge_attr=None):
        if self.node_feature_only:
            if self.sparse:
                return self.sparse_forward_node_feature_only(
                    x, timesteps, edge_index, edge_attr
                )
            else:
                return self.dense_forward_node_feature_only(x, graph, timesteps)
        else:
            if self.sparse:
