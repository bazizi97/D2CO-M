...
class ScalarEmbeddingSine1D(nn.Module):
  ...
  def forward(self, x):
    ...
    pos_x = torch.stack((pos_x[:, :, 0::2].sin(), pos_x[:, :, 1::2].cos()), dim=3)  # Shape: (32, 20, 128, 2)
...
class GNNEncoder(nn.Module):
  ...
  def __init__(self, n_layers, hidden_dim, n_nodes_features=2, n_edges_features=4,
    ...
    self.out = nn.Sequential(
        ...
        nn.ReLU(),
        nn.Linear(hidden_dim, n_nodes_features)
    )
  ...
  def forward(self, x, timesteps, graph=None, edge_index=None, edge_attr=None):
    if self.node_feature_only:
      if self.sparse:
        return self.sparse_forward_node_feature_only(x, timesteps, edge_index, edge_attr)
      else:
